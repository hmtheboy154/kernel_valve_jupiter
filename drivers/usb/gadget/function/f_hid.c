...
struct f_hidg {
	...
	/* set report */
	struct list_head		completed_set_report_req;
	spinlock_t			set_report_spinlock;
	wait_queue_head_t		set_report_queue;

	/* get report */
	struct usb_request		*get_req;
	struct usb_hidg_report		get_report;
	spinlock_t			get_report_spinlock;
	bool				get_report_pending;
	wait_queue_head_t		get_report_queue;
...
static int f_hidg_set_report(struct file *file, struct usb_hidg_report __user *buffer)
{
	...
	spin_lock_irqsave(&hidg->set_report_spinlock, flags);

#define SET_REPORT_COND (!list_empty(&hidg->completed_set_report_req))

	/* wait for at least one buffer to complete */
	while (!SET_REPORT_COND) {
		spin_unlock_irqrestore(&hidg->set_report_spinlock, flags);
		if (file->f_flags & O_NONBLOCK)
			return -EAGAIN;

		if (wait_event_interruptible(hidg->set_report_queue, SET_REPORT_COND))
			return -ERESTARTSYS;
	...
	/* pick the first one */
	list = list_first_entry(&hidg->completed_set_report_req,
				struct f_hidg_req_list, list);
...
#define GET_REPORT_COND (!hidg->get_report_pending)
	...
	while (!GET_REPORT_COND) {
		...
		if (wait_event_interruptible_exclusive(hidg->get_report_queue,
						       GET_REPORT_COND))
			return -ERESTARTSYS;
	...
	spin_lock_irqsave(&hidg->get_report_spinlock, flags);
	hidg->get_report_pending = false;
	spin_unlock_irqrestore(&hidg->get_report_spinlock, flags);

	wake_up(&hidg->get_report_queue);
	return status;
}
...
static __poll_t f_hidg_poll(struct file *file, poll_table *wait)
{
	...
	poll_wait(file, &hidg->set_report_queue, wait);
...
static void hidg_set_report_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	case 0:
		...
		if (ep == cdev->gadget->ep0) {
			spin_lock_irqsave(&hidg->set_report_spinlock, flags);
			list_add_tail(&req_list->list, &hidg->completed_set_report_req);
			spin_unlock_irqrestore(&hidg->set_report_spinlock, flags);

			wake_up(&hidg->set_report_queue);
		} else {
			spin_lock_irqsave(&hidg->read_spinlock, flags);
...
static void hidg_disable(struct usb_function *f)
{
	...
	spin_lock_irqsave(&hidg->set_report_spinlock, flags);
	list_for_each_entry_safe(list, next, &hidg->completed_set_report_req, list) {
...
static int hidg_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	spin_lock_init(&hidg->set_report_spinlock);
	spin_lock_init(&hidg->get_report_spinlock);
	init_waitqueue_head(&hidg->write_queue);
	init_waitqueue_head(&hidg->read_queue);
	init_waitqueue_head(&hidg->set_report_queue);
	init_waitqueue_head(&hidg->get_report_queue);
	INIT_LIST_HEAD(&hidg->completed_out_req);
	INIT_LIST_HEAD(&hidg->completed_set_report_req);
